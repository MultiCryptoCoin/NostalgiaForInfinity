...
class Obelisk_Ichimoku_ZEMA_v1(IStrategy):
    ...
    def slow_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[:, "cloud_top"] = dataframe.loc[:, ["senkou_a", "senkou_b"]].max(
            axis=1
        )
    ...
    def fast_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        if self.config["runmode"].value == "hyperopt":
            ...
            dataframe[f"zema_buy"] = (
                ftt.zema(dataframe, period=self.zema_len_buy.value)
                * self.low_offset.value
            )
            dataframe[f"zema_sell"] = (
                ftt.zema(dataframe, period=self.zema_len_sell.value)
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe.loc[
            ((dataframe["close"] > (dataframe[zema] * self.high_offset.value))), "sell"
        ] = 1
