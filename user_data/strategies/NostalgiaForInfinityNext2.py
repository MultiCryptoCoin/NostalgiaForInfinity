...
class NostalgiaForInfinityNext2(IStrategy):
    ...
    buy_mfi_1 = DecimalParameter(
        20.0, 40.0, default=44.0, space="buy", decimals=1, optimize=False, load=True
    )
    ...
    buy_chop_min_19 = DecimalParameter(
        20.0, 60.0, default=26.1, space="buy", decimals=1, optimize=False, load=True
    )
    ...
    def load_hold_trades_config(self):
        if (
            self.hold_trade_ids is not None
            and self.hold_trade_ids_profit_ratio is not None
        ):
            # Already loaded
            return
        ...
        with hold_trades_config_file.open("r") as f:
            try:
                ...
                formatted_profit_ratio = "{}%".format(
                    self.hold_trade_ids_profit_ratio * 100
                )
