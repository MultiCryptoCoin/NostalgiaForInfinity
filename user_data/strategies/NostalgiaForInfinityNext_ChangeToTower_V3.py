...
class NostalgiaForInfinityNext_ChangeToTower_V3(IStrategy):
    ...
    def normal_tf_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["crsi"] = (
            dataframe["rsi5"] + dataframe["srsi"] + dataframe["roc"]
        ) / 3
...
def KalmanFilter(dtloc, source="close"):
    ...
    def calc_dtKF(dfr, init=0):
        ...
        if init == 1:
            ...
            return
        calc_dtKF_value_1 = (
            0.2 * (dfr[source] - calc_dtKF_source) + 0.8 * calc_dtKF_value_1
        )
        ...
        if calc_dtKF_value_2 != 0:
            ...
            vlambda = 0
        valpha = (
            -1 * math.pow(vlambda, 2)
            + math.sqrt(math.pow(vlambda, 4) + 16 * math.pow(vlambda, 2))
        ) / 8
