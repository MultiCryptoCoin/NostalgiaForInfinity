...
pairs: # pull pairs for $COIN
	docker-compose run --rm freqtrade \
		list-pairs --config user_data/test.json --quot=$(COIN) --print-json

list-data: # list data
	docker-compose run --rm freqtrade \
		 list-data --config user_data/test.json
...
test: # run backtest
	docker-compose run --rm freqtrade \
		backtesting --config user_data/test.json --strategy-list $(STRATEGY) --timeframe $(B_TIMEFRAME) --timerange=$(TIMERANGE) \
		--export=trades
	osascript -e 'display notification "Done"'

test-all: # run backtest on all strats
	docker-compose run --rm freqtrade \
		backtesting --config user_data/test.json --strategy-list $(STRATEGIES) --timerange=$(TIMERANGE) --timeframe $(B_TIMEFRAME) --export=trades
	osascript -e 'display notification "Done"'

hyperopt: data # run hyper opt
	docker-compose run --rm freqtrade \
		hyperopt --config user_data/test.json --hyperopt-loss $(LOSS) --spaces $(SPACES) --strategy $(STRATEGY) -e $(EPOCHS) --timerange=$(TIMERANGE) --timeframe=$(TIMEFRAME) --random-state 42 -j 12
	osascript -e 'display notification "Done"'
